...
contract UserDefValueType {
  function example_no_uvdt() public view {
    // Without UDVT
    uint128 clock;
    uint64 d = 1;
  ...
  function example_uvdt() public view {
    // Turn primitive value types into user defined value types
    Duration d = Duration.wrap(1);
    Timestamp t = Timestamp.wrap(uint64(block.timestamp));

    // Turn user defined value types back to primitive value type
